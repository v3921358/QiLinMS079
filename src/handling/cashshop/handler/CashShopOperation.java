...
public class CashShopOperation {
    ...
    public static void EnterCashShop(final int playerid, final MapleClient client) {
        ...
        if (!LoginServer.CanLoginKey(client.getPlayer().getLoginKey(), client.getPlayer().getAccountID()) || (LoginServer.getLoginKey(client.getPlayer().getAccountID()) == null && !client.getPlayer().getLoginKey().isEmpty())) {
            FileoutputUtil.logToFile("logs/Data/客戶端登錄KEY異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + client.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + client.getAccountName() + " 客戶端key：" + LoginServer.getLoginKey(client.getPlayer().getAccountID()) + " 伺服端key：" + client.getPlayer().getLoginKey() + " 進入商城1");
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 非法登錄 账号 " + client.getAccountName()));
            client.getSession().close();
            return;
        }
        if (!LoginServer.CanServerKey(client.getPlayer().getServerKey(), client.getPlayer().getAccountID()) || (LoginServer.getServerKey(client.getPlayer().getAccountID()) == null && !client.getPlayer().getServerKey().isEmpty())) {
            FileoutputUtil.logToFile("logs/Data/客戶端頻道KEY異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + client.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + client.getAccountName() + " 客戶端key：" + LoginServer.getServerKey(client.getPlayer().getAccountID()) + " 伺服端key：" + client.getPlayer().getServerKey() + " 進入商城2");
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 非法登錄 账号 " + client.getAccountName()));
            client.getSession().close();
            return;
        }
        if (!LoginServer.CanClientKey(client.getPlayer().getClientKey(), client.getPlayer().getAccountID()) || (LoginServer.getClientKey(client.getPlayer().getAccountID()) == null && !client.getPlayer().getClientKey().isEmpty())) {
            FileoutputUtil.logToFile("logs/Data/客戶端進入KEY異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + client.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + client.getAccountName() + " 客戶端key：" + LoginServer.getClientKey(client.getPlayer().getAccountID()) + " 伺服端key：" + client.getPlayer().getClientKey() + " 進入商城3");
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 非法登錄 账号 " + client.getAccountName()));
            client.getSession().close();
            return;
        }
       ...
       // }
        if (client.getPlayer().getCharacterNameById2(playerid) == null) {
            ...
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 非法登錄不存在角色 账号 " + client.getAccountName()));
            client.getSession().close();
            return;
        }
        ...
        if (validcode) {
            ...
            switch (as) {
                case 1:
                    ...
                    c.getPlayer().dropMessage(1, "已成功使用優待卷获得" + MapleItemInformationProvider.getInstance().getName(item) + time + "天 x" + size + "。");
                    break;
                case 2:
                    c.getPlayer().dropMessage(1, "已成功使用優待卷获得" + MapleItemInformationProvider.getInstance().getName(item) + "永久 x" + size + "。");
                    break;
                default:
                    c.getPlayer().dropMessage(1, "已成功使用優待卷获得" + item + cc);
                    break;
            }
        } else {
    ...
    public static final void BuyCashItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (action) {
                    ...
                    if (action == 3) { // 购买單個道具
                        ...
                        if (itemz != null && itemz.getUniqueId() > 0 && itemz.getItemId() == cItem.getId() && itemz.getQuantity() == cItem.getCount()) {
                            ...
                            if (ServerConfig.LOG_CSBUY) {
                                FileoutputUtil.logToFile("logs/Data/商城购买.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了" + (useNX == 1 ? "點券" : "抵用卷") + cItem.getPrice() + "點 來购买" + cItem.getId() + "x" + cItem.getCount());
                            }
                        } else {
                        ...
                        if (ServerConfig.LOG_CSBUY) {
                            FileoutputUtil.logToFile("logs/Data/商城购买.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了" + (useNX == 1 ? "點券" : "抵用卷") + cItem.getPrice() + "點 來购买套裝" + cItem.getId() + "x" + cItem.getCount());
                        }
                        chr.modifyCSPoints(useNX, -cItem.getPrice(), false);
            ...
            case 4: { // gift
                ...
                if (canBuy && info != null && cItem != null) {
                    ...
                    if (ServerConfig.LOG_CSBUY) {
                        FileoutputUtil.logToFile("logs/Data/商城送禮.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了點券" + cItem.getPrice() + "點 贈送了" + cItem.getId() + "x" + cItem.getCount() + " 給" + characterName);
                    }
            ...
            case 6: {
                ...
                if (coupon) {
                    ...
                    if (chr.getCSPoints(useNX) >= (ServerConfig.DISCOUNTED ? 540 : 600) && chr.getInventory(type).getSlotLimit() < 89) {
                        ...
                        chr.dropMessage(1, "栏位已經擴充到 " + chr.getInventory(type).getSlotLimit());
                        if (ServerConfig.LOG_CSBUY) {
                            FileoutputUtil.logToFile("logs/Data/商城擴充.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了" + (useNX == 1 ? "點券" : "抵用卷") + "100點 來购买擴充栏位" + type.name() + "8格 目前共有" + chr.getInventory(type).getSlotLimit() + "格");
                        }
                    } else {
                    ...
                    if (chr.getCSPoints(useNX) >= (ServerConfig.DISCOUNTED ? 540 : 600) && chr.getInventory(type).getSlotLimit() < 93) {
                        ...
                        chr.dropMessage(1, "栏位已經擴充到 " + chr.getInventory(type).getSlotLimit());
                        if (ServerConfig.LOG_CSBUY) {
                            FileoutputUtil.logToFile("logs/Data/商城擴充.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了" + (useNX == 1 ? "點券" : "抵用卷") + "100點 來购买擴充栏位" + type.name() + "4格 目前共有" + chr.getInventory(type).getSlotLimit() + "格");
                        }
                    } else {
                        c.sendPacket(MTSCSPacket.sendCSFail(0xA4));
                    }
            ...
            case 7: {
                ...
                if (chr.getCSPoints(useNX) >= (ServerConfig.DISCOUNTED ? 540 : 600) && chr.getStorage().getSlots() < 45) {
                    ...
                    if (ServerConfig.LOG_CSBUY) {
                        FileoutputUtil.logToFile("logs/Data/商城擴充.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了" + (useNX == 1 ? "點券" : "抵用卷") + "100點 來购买擴充栏位仓库4格 目前共有" + chr.getStorage().getSlots() + "格");
                    }
            ...
            case 8: {
                ...
                if (c.gainCharacterSlot()) {
                    ...
                    chr.dropMessage(1, "角色栏位已經擴充到 " + c.getCharacterSlots());
                    if (ServerConfig.LOG_CSBUY) {
            ...
            case 13: {//商城拿出
                ...
                if (item != null && item.getQuantity() > 0 && MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), item.getOwner())) {
                    ...
                    if (pos >= 0) {
                        ...
                        FileoutputUtil.logToFile("logs/Data/商城拿出.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 從商城拿出 " + item_.getItemId() + "x" + item_.getQuantity());
                    } else {
            ...
            case 14: {//商城存入
                ...
                if (item != null && item.getQuantity() > 0 && item.getUniqueId() > 0 && c.getPlayer().getCashInventory().getItemsSize() < 100) {
                    ...
                    //item_.setGMLog("購物商城购买 时间: " + FileoutputUtil.CurrentReadable_Time());
                    c.getPlayer().getCashInventory().addToInventory(item_);
                    c.sendPacket(MTSCSPacket.confirmToCSInventory(item, c.getAccID(), sn));
                    FileoutputUtil.logToFile("logs/Data/商城存入.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 從商城存入 " + item_.getItemId() + "x" + item_.getQuantity());
                } else {
                    c.sendPacket(MTSCSPacket.sendCSFail(0xB1));
                }
                RefreshCashShop(c);
        ...
        case 26: { //換購
            ...
            chr.dropMessage(1, "成功换购抵用券" + Money + "点。");
              if (ServerConfig.LOG_CSBUY) {
                            FileoutputUtil.logToFile("logs/Data/商城換購.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了 "+item.getItemId()+" 換購获得抵用卷 "+Money);
                        }
            RefreshCashShop(c);
                break;
             }
            ...
            case 36: { // friendRing
                ...
                if (canBuy && info != null && cItem != null) {
                    ...
                    if (ServerConfig.LOG_CSBUY) {
                        FileoutputUtil.logToFile("logs/Data/商城送禮.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了點券" + cItem.getPrice() + "點 贈送了" + cItem.getId() + "x" + cItem.getCount() + " 給" + partnerName);
                    }
                } else {
            ...
            case 51: { //抵用卷购买
                ...
                chr.dropMessage(1, "成功购买抵用卷:" + item.getPrice());
