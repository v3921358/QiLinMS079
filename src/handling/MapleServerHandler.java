...
public class MapleServerHandler extends ChannelInboundHandlerAdapter {
            ...
            if ((!(cause instanceof IOException))) {
                ...
                if ((client != null) && (client.getPlayer() != null)) {
                    try {
                        ...
                        FileoutputUtil.logToFile("logs/異常拋出保存数据異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + client.getSession().remoteAddress().toString().split(":")[0] + " 账号 " + client.getAccountName() + " 账号ID " + client.getAccID() + " 角色名 " + client.getPlayer().getName() + " 角色ID " + client.getPlayer().getId());
                        FileoutputUtil.outError("logs/異常拋出保存数据異常.txt", eex);
                    }
                    FileoutputUtil.printError("logs/發現異常.txt", cause, "發現異常 by: 玩家:" + client.getPlayer().getName() + " 职业:" + client.getPlayer().getJob() + " 地图:" + client.getPlayer().getMap().getMapName() + " - " + client.getPlayer().getMapId());
                }
            }
        } catch (Exception e) {
    ...
    public void channelRead(final ChannelHandlerContext ctx, final Object message) {
            ...
            if (recv.getValue() == header_num) {
                ...
                try {
                    ...
                    if (c.getPlayer() != null && c.getPlayer().isShowErr()) {
                        c.getPlayer().showInfo("数据包異常", true, "包頭:" + recv.name() + "(0x" + Integer.toHexString(header_num).toUpperCase() + ")");
                    }
        ...
        if (ServerConfig.LOG_PACKETS) {
            ...
            final StringBuilder sb = new StringBuilder("發現未知用戶端数据包 - (包頭:0x" + Integer.toHexString(header_num) + ")");
        ...
        switch (header) {
            ...
            case ENTER_CASH_SHOP:
                if (c.getPlayer().getMapId() != 980000010
                    ...
                    c.getPlayer().dropMessage(5, "該地图無法進入商城。");
                }
                break;
            case ENTER_MTS:
                if (c.getPlayer().getMapId() == 910010300 || c.getPlayer().getMapId() == 910010100 || ((c.getPlayer().getMapId() > 211060000) && (c.getPlayer().getMapId() <= 211061000))) {
                    c.getPlayer().dropMessage(5, "該地图無法使用拍賣功能。");
                } else {
                    c.getSession().writeAndFlush(tools.MaplePacketCreator.enableActions());
...
//                System.err.println("[用戶端 報錯] 錯誤代码 " + errortype + " 類型 " + type_str + "\r\n\t[数据] 長度 " + data_length + " [" + SendPacketOpcode.nameOf(opcodeheader) + " | " + opcodeheader + "]\r\n" + HexTool.toString(slea.read((int) slea.available())));
                String tab = "";
            ...
            case LICENSE_REQUEST:
                ...
                break;
            default:
                //System.err.println("[發現未處理数据包] Recv [" + header.toString() + "]");
                break;
        }
    }
}
