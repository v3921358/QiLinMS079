...
//        closeAllMerchant();
    ...
    public final void addPlayer(final MapleCharacter chr) { 
        ...
        if (gui.Qhms.ConfigValuesMap.get("滚动公告开关") <= 0) {
...
//开服名字
    ...
    public final void setPrepareShutdown() {
        this.shutdown = true;
        System.out.println("【頻道" + getChannel() + "】 准备关闭.");
    }

    public final void setFinishShutdown() {
        this.finishedShutdown = true;
        System.out.println("【頻道" + getChannel() + "】 已經关闭完成.");
    }
    ...
    public void saveAll() {
        ...
        for (MapleCharacter chr : all) {
            try {
                ...
                FileoutputUtil.outError("logs/saveAll存檔保存数据異常.txt", e);
    ...
    public static void forceRemovePlayerByAccId(MapleClient client, int accid) {
        ...
        try {
            ...
            for (MapleCharacter c : chrs) {
                if (c.getAccountID() == accid) {
                    try {
                        //   FileoutputUtil.logToFile("logs/Hack/洗道具.txt", "\r\n" + FileoutputUtil.NowTime() + " MAC: " + client.getMacs() + " IP: " + client.getSessionIPAddress() + " 账号: " + accid + " 角色: " + c.getName(), false, false);
                        if (c.getClient() != null) {
                            if (c.getClient() != client) {
                                c.getClient().unLockDisconnect();
                            }
