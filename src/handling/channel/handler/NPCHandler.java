...
public class NPCHandler {
    ...
    public static final void handleNPCAnimation(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (npc == null) {
            ...
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().showInfo("NPC動作", true, "地图上不存在NPC, OID=" + oid);
            }
            return;
        }
    ...
    public static final void handleNPCTalk(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getConversation() != 0) {
            chr.dropMessage(5, "你現在不能攻击或不能跟npc對話,請在對話框打 @解卡/@ea 來解除異常状态");
            return;
        }
                ...
                if (c.getPlayer().CanStorage()) {
                    if (item != null) {
                        if (!MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), item.getOwner())) {
                            ...
                            FileoutputUtil.logToFile("logs/Data/仓库操作.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了仓库操作拿出物品:" + item.getItemId() + " 數量：" + item.getQuantity());
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] " + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了仓库操作拿出物品:" + item.getItemId() + " 數量：" + item.getQuantity()));
                        }
                    } else {
                        c.sendPacket(MaplePacketCreator.enableActions());
                ...
                if (c.getPlayer().CanStorage()) {
                    ...
                    if (chr.getMeso() < 100 || chr.getMapId() == 910000000 && chr.getMeso() < 500) {
                        ...
                        if (item.getItemId() == itemId && (item.getQuantity() >= quantity || GameConstants.isThrowingStar(itemId) || GameConstants.isBullet(itemId))) {
                            ...
                            //chr.saveToDB(false, false);
                            FileoutputUtil.logToFile("logs/Data/仓库操作.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了仓库操作存入物品:" + item.getItemId() + " 數量：" + item.getQuantity());
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] " + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了仓库操作存入物品:" + item.getItemId() + " 數量：" + item.getQuantity()));
                        } else {
            ...
            case 7: {
                ...
                if (c.getPlayer().CanStorage()) {
                    if ((meso > 0 && storageMesos >= meso) || (meso < 0 && playerMesos >= -meso)) {
                        ...
                        FileoutputUtil.logToFile("logs/Data/仓库操作.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了仓库操作金幣數量:" + meso);
                        World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] " + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了仓库操作金幣數量:" + meso));

                        if (meso >= 50000000 || meso <= -50000000) {
                            FileoutputUtil.logToFile("logs/Data/仓库大額金幣操作.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了仓库大額金幣操作數量:" + meso);
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] " + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了仓库大額金幣操作數量:" + meso));
                        }
