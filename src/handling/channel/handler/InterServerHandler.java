...
public class InterServerHandler {
    ...
    public static final void EnterCashShop(final MapleClient c, final MapleCharacter chr, final boolean mts) {
        ...
        try {
            ...
            FileoutputUtil.outError("logs/進入商城保存数据異常.txt", ex);
        }
    ...
    public static final void LoggedIn(final int playerid, final MapleClient c) {
            ...
            for (ChannelServer cs : ChannelServer.getAllInstances()) {
                for (final String name : charNamesa) {
                    if (cs.getPlayerStorage().getCharacterByName(name) != null) {
                        ...
                        World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 非法登錄 账号 " + c.getAccountName()));
                        c.getSession().close();
                        return;
                    }
                }
        ...
        if (!c.CheckIPAddress()) { // Remote hack
            c.getSession().close();
            FileoutputUtil.logToFile("logs/Data/進入游戏掉線.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " CheckIPAddress");
            return;
        }
        ...
        if (player.getCharacterNameById2(playerid) == null) {
            ...
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 非法登錄不存在角色 账号 " + c.getAccountName()));
            c.getSession().close();
            return;
        }
        ...
        //管理員上線預設隱藏
        int 管理隐身 = gui.Qhms.ConfigValuesMap.get("管理隐身开关");
        if (管理隐身 <= 0) {//管理隐身
        if (player.isGM()) {
            SkillFactory.getSkill(9001004).getEffect(1).applyTo(player);
        int 管理加速 = gui.Qhms.ConfigValuesMap.get("管理加速开关");
        if (管理加速 <= 0) {//管理加速
        ...
        if (c.getPlayer().hasEquipped(1122017)) {
           ...
           if (gui.Qhms.ConfigValuesMap.get("上线提醒开关") <= 0) {
           if (player.getGender() == 0) {
                   World.Broadcast.broadcastSmega(MaplePacketCreator.serverNotice(0x0C, c.getChannel(), "[登录公告] 【帅哥】" + player.getName() + " : " +"进入游戏，大家热烈欢迎他吧！！！"));
                } else {
                   World.Broadcast.broadcastSmega(MaplePacketCreator.serverNotice(0x0C, c.getChannel(), "[登录公告] 【美女】" + player.getName() + " : " +"进入游戏，大家热烈欢迎她吧！！！"));
                }       
           }

        player.updatePetAuto();
        
        if (gui.Qhms.ConfigValuesMap.get("登陆帮助开关") == 0) {
        ...
        if (gui.Qhms.ConfigValuesMap.get("幸运职业开关") == 0) {
        ...
        if (ChrdangerousIp) {
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 危險IP上線" + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + player.getName() + " 角色ID " + player.getId()));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 危險IP上線" + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + player.getName() + " 角色ID " + player.getId()));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 危險IP上線" + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + player.getName() + " 角色ID " + player.getId()));
        ...
        if (ChrdangerousName) {
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 危險角色上線" + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + player.getName() + " 角色ID " + player.getId()));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 危險角色上線" + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + player.getName() + " 角色ID " + player.getId()));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 危險角色上線" + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + player.getName() + " 角色ID " + player.getId()));
    ...
    public static final void ChangeChannel(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        /*if (!LoginServer.CanLoginKey(chr.getLoginKey(), chr.getAccountID()) || (LoginServer.getLoginKey(chr.getAccountID()) == null && !chr.getLoginKey().isEmpty())) {
            FileoutputUtil.logToFile("logs/Data/客戶端登錄KEY異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 客戶端key：" + LoginServer.getLoginKey(chr.getAccountID()) + " 伺服端key：" + chr.getLoginKey() + " 更換頻道1");
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 非法更換頻道 账号 " + c.getAccountName()));
            c.getSession().close();
            return;
        }
        if (!LoginServer.CanServerKey(chr.getServerKey(), chr.getAccountID()) || (LoginServer.getServerKey(chr.getAccountID()) == null && !chr.getServerKey().isEmpty())) {
            FileoutputUtil.logToFile("logs/Data/客戶端頻道KEY異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 客戶端key：" + LoginServer.getServerKey(chr.getAccountID()) + " 伺服端key：" + chr.getServerKey() + " 更換頻道2");
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 非法更換頻道 账号 " + c.getAccountName()));
            c.getSession().close();
            return;
        }
        if (!LoginServer.CanClientKey(chr.getClientKey(), chr.getAccountID()) || (LoginServer.getClientKey(chr.getAccountID()) == null && !chr.getClientKey().isEmpty())) {
            FileoutputUtil.logToFile("logs/Data/客戶端進入KEY異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 客戶端key：" + LoginServer.getClientKey(chr.getAccountID()) + " 伺服端key：" + chr.getClientKey() + " 更換頻道3");
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 非法更換頻道 账号 " + c.getAccountName()));
            c.getSession().close();
            return;
        }*/
