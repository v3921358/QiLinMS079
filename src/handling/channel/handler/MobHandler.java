...
public class MobHandler {
    ...
    public static final void MoveMonster(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        try {
            ...
            //  World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] " + chr.getName() + " (編號: " + chr.getId() + ") 怪物移動錯誤(" + monster.getId() + ")! - 地图:" + chr.getMapId()), true);
                ...
                if (!fly) {
                    ...
                    if (((reduce_x > GeneallyDistance_x || reduce_y > GeneallyDistance_y) && reduce_y != 0) || (reduce_x > Check_x && reduce_y == 0) || reduce_x > max_x) {
                        ...
                        if (c.getPlayer().get吸怪() % 50 == 0 || reduce_x > max_x) {
                            ...
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] " + chr.getName() + " (編號: " + chr.getId() + ")使用吸怪(" + chr.get吸怪() + ")! - 地图:" + chr.getMapId() + "(" + chr.getMap().getMapName() + ")"), true);
