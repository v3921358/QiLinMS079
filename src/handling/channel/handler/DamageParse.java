...
public class DamageParse {
    ...
    public static void applyAttack(final AttackInfo attack, final ISkill theSkill, final MapleCharacter player, int attackCount, final double maxDamagePerMonster, final MapleStatEffect effect, final AttackType attack_type) {
        ...
        if (attack.skill != 0) {
            ...
            if (attack.hits > last) {
                if (player.hasGmLevel(1)) {
                    player.dropMessage("攻击次數異常攻击次數 " + attack.hits + " 服務端判斷正常攻击次數 " + last + " 技能ID " + attack.skill);
                } else {
                    player.ban(player.getName() + "技能攻击次數異常", true, true, false);
                    ...
                    World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] " + player.getName() + " (等級 " + player.getLevel() + ") 攻击次數異常已自動封鎖。 玩家攻击次數 " + attack.hits + " 服務端判斷正常攻击次數 " + last + " 技能ID " + attack.skill));
                    FileoutputUtil.logToFile("logs/Hack/Ban/技能攻击次數.txt", "\r\n" + FileoutputUtil.NowTime() + "玩家: " + player.getName() + "(" + player.getLevel() + ") 地图: " + player.getMapId() + " 技能代码: " + attack.skill + " 技能等級: " + player.getSkillLevel(attack.skill) + " 攻击次數 : " + attack.hits + " 正常攻击次數 :" + last);
                    return;
                }
            }
            ...
            if (attack.targets > CheckCount) {
                if (player.hasGmLevel(1)) {
                    player.dropMessage("打怪數量異常,技能代码: " + attack.skill + " 封包怪物量 : " + attack.targets + " 服務端怪物量 :" + CheckCount);
                } else {
                    FileoutputUtil.logToFile("logs/Hack/Ban/打怪數量異常.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 地图: " + player.getMapId() + "技能代码: " + attack.skill + " 技能等級: " + player.getSkillLevel(effect.getSourceId()) + " 封包怪物量 : " + attack.targets + " 服務端怪物量 :" + CheckCount);
                    player.ban(player.getName() + "打怪數量異常", true, true, false);
                    player.getClient().getSession().close();
                    String reason = "使用違法程式練功";
                    World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + player.getName() + " 因為" + reason + "而被管理員永久停權。"));
                    World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] " + player.getName() + " (等級 " + player.getLevel() + ") " + "攻击怪物數量異常。 " + "封包怪物量 " + attack.targets + " 服務端怪物量 " + CheckCount + " 技能ID " + attack.skill));
                    return;
                }
            }
        ...
        if (attack.skill == SkillType.神偷.金币炸彈) { // meso explosion
        ...
        for (final AttackPair oned : attack.allDamage) {
            ...
            if (monster != null) {
                    ...
                    if (!GameConstants.isElseSkill(attack.skill)) {
                        if (GameConstants.Novice_Skill(attack.skill)) {//新手技能
                            if (eachd > 40) {
                                ...
                                FileoutputUtil.logToFile("logs/Hack/Ban/伤害異常.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家<" + player.getLevel() + ">: " + player.getName() + " 怪物 " + monster.getId() + " 地图: " + player.getMapId() + " 技能代码: " + attack.skill + " 最高伤害: 40 本次伤害 :" + eachd + " 預計伤害: " + (int) maxDamagePerHit + "是否為BOSS: " + monster.getStats().isBoss() + " 紫色蘋果: " + apple);
                                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + player.getName() + " 因為伤害異常而被管理員永久停權。"));
                                World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] " + player.getName() + " (等級 " + player.getLevel() + ") " + "伤害異常。 " + "最高伤害 40 本次伤害 " + eachd + " 技能ID " + attack.skill));
                                player.ban(player.getName() + "伤害異常", true, true, false);
                                player.getClient().getSession().close();
                                return;
                            }
                        }
                        ...
                        if (!GameConstants.isAran(player.getJob())) {
                            ...
                            // maxDamagePerHit 利用 defined 變數控制是否超過19萬攻击
                            ...
                            if (ban) {
                                ...
                                FileoutputUtil.logToFile("logs/Hack/Ban/伤害異常.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家<" + player.getLevel() + ">: " + player.getName() + " 怪物 " + monster.getId() + " 地图: " + player.getMapId() + " 技能代码: " + attack.skill + " 最高伤害: " + atk + " 本次伤害 :" + eachd + " 預計伤害: " + (int) maxDamagePerHit + "是否為BOSS: " + monster.getStats().isBoss() + " 紫色蘋果: " + apple);
                                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + player.getName() + " 因為伤害異常而被管理員永久停權。"));
                                World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] " + player.getName() + " (等級 " + player.getLevel() + ") " + "伤害異常。 " + "最高伤害 " + atk + " 本次伤害 " + eachd + " 技能ID " + attack.skill));
                                player.ban(player.getName() + "伤害異常", true, true, false);
                                player.getClient().getSession().close();
                                return;
                            }
                        ...
                        if (GameConstants.isAran(player.getJob())) {
                            if (player.getLevel() > 10) {
                                if (eachd > atk && eachd > maxDamagePerHit * 2) {
                                    FileoutputUtil.logToFile("logs/Hack/伤害異常_狂狼.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家<" + player.getLevel() + ">: " + player.getName() + " 技能代码: " + attack.skill + " 怪物: " + monster.getId() + " 最高伤害: " + atk + " 本次伤害 :" + eachd + " 預計伤害: " + (int) maxDamagePerHit + "是否為BOSS: " + monster.getStats().isBoss());
                                }
                            }
                    ...
                    if (fixeddmg != -1) {
                        ...
                        if (Tempest) { // Monster buffed with Tempest
                            ...
                            if (eachd > maxDamagePerHit) {
                                player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE, new StringBuilder().append("[伤害: ").append(eachd).append(", 預期: ").append(maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [职业: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 使用的技能: ").append(attack.skill).append("]").toString());
                                if (eachd > maxDamagePerHit * 2) {
                                    if (eachd > maxDamagePerHit * 2.0D && maxDamagePerHit != 1) {
                                        if (ServerConfig.LOG_DAMAGE) {
                                            FileoutputUtil.logToFile("Logs/hack/伤害計算/伤害計算修正_" + monster.getId() + "_" + attack.skill + ".txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 职业: " + player.getJob() + " 怪物:" + monster.getId() + " 封包伤害 :" + eachd + " 預計伤害 :" + (int) maxDamagePerHit + " 是否為BOSS: " + monster.getStats().isBoss(), false, false);
                                        }
                                        player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_2, new StringBuilder().append("[伤害: ").append(eachd).append(", 預計伤害: ").append((int) maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [职业: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 技能: ").append(attack.skill).append("]").toString());
                                    }
                                    eachd = (int) (maxDamagePerHit * 2); // Convert to server calculated damage
                                    player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_2, new StringBuilder().append("[伤害: ").append(eachd).append(", 預期: ").append(maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [职业: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 使用的技能: ").append(attack.skill).append("]").toString());
                                    if (eachd >= 10000) {
                                        player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_2, new StringBuilder().append("[伤害: ").append(eachd).append(", 預期: ").append(maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [职业: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 使用的技能: ").append(attack.skill).append("]").toString());
                                    }
                                }
                            ...
                            if (eachd > maxDamagePerHit * 2.0D && maxDamagePerHit != 1) {
                                if (ServerConfig.LOG_DAMAGE) {
                                    FileoutputUtil.logToFile("Logs/hack/伤害計算/伤害計算修正_" + monster.getId() + "_" + attack.skill + ".txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 职业: " + player.getJob() + " 怪物:" + monster.getId() + " 封包伤害 :" + eachd + " 預計伤害 :" + (int) maxDamagePerHit + " 是否為BOSS: " + monster.getStats().isBoss());
                                }
                                player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_2, new StringBuilder().append("[伤害: ").append(eachd).append(", 預計伤害: ").append((int) maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [职业: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 技能: ").append(attack.skill).append("]").toString());
                            }
                        }
                ...
                if (range > SkillRange && !player.inBossMap()) { // 815^2 <-- the most ranged attack in the game is Flame Wheel at 815 range
                    player.getCheatTracker().registerOffense(CheatingOffense.ATTACK_FARAWAY_MONSTER, "攻击範圍異常,技能:" + attack.skill + "(" + SkillFactory.getName(attack.skill) + ")　怪物:" + monster.getId() + " 正常範圍:" + (int) SkillRange + " 計算範圍:" + (int) range); // , Double.toString(Math.sqrt(distance))
                    if (range > SkillRange * 2) {
                        player.getCheatTracker().registerOffense(CheatingOffense.ATTACK_FARAWAY_MONSTER_BAN, "超大攻击範圍,技能:" + attack.skill + "(" + SkillFactory.getName(attack.skill) + ")　怪物:" + monster.getId() + " 正常範圍:" + (int) SkillRange + " 計算範圍:" + (int) range); // , Double.toString(Math.sqrt(distance))
                    }
                    return;
                }
                // pickpocket
    ...
    public static final void applyAttackMagic(final AttackInfo attack, final ISkill theSkill, final MapleCharacter player, final MapleStatEffect effect) {
        ...
        if (attack.hits > last) {
            if (player.hasGmLevel(1)) {
                ...
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + player.getName() + " 因為" + reason + "而被管理員永久停權。"));
                World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] " + player.getName() + " (等級 " + player.getLevel() + ") 攻击次數異常已自動封鎖。 玩家攻击次數 " + attack.hits + " 服務端判斷正常攻击次數 " + last + " 技能ID " + attack.skill));
                FileoutputUtil.logToFile("logs/Hack/Ban/技能攻击次數.txt", "\r\n" + FileoutputUtil.NowTime() + "玩家: " + player.getName() + "(" + player.getLevel() + ") 技能代码: " + attack.skill + " 技能等級: " + player.getSkillLevel(attack.skill) + " 攻击次數 : " + attack.hits + " 正常攻击次數 :" + last);
                return;
            }
        }
        /* 確認是否超過打怪數量*/
        ...
        if (attack.targets > CheckCount) {
            if (player.hasGmLevel(1)) {
                ...
                FileoutputUtil.logToFile("logs/Hack/Ban/打怪數量異常.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 技能代码: " + attack.skill + " 技能等級: " + player.getSkillLevel(effect.getSourceId()) + " 封包怪物量 : " + attack.targets + " 服務端怪物量 :" + CheckCount);
                player.ban(player.getName() + "打怪數量異常", true, true, false);
                player.getClient().getSession().close();
                String reason = "使用違法程式練功";
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + player.getName() + " 因為" + reason + "而被管理員永久停權。"));
            ...
            if (monster != null) {
                ...
                for (Pair<Integer, Boolean> eachde : oned.attack) {
                    ...
                    if (!GameConstants.isElseSkill(attack.skill)) {
                        if (GameConstants.Novice_Skill(attack.skill)) {//新手技能
                            if (eachd > 40) {
                                FileoutputUtil.logToFile("logs/Hack/Ban/伤害異常.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家<" + player.getLevel() + ">: " + player.getName() + " 怪物 " + monster.getId() + " 地图: " + player.getMapId() + " 技能代码: " + attack.skill + " 最高伤害: 40 本次伤害 :" + eachd + " 預計伤害: " + (int) maxDamagePerHit + "是否為BOSS: " + monster.getStats().isBoss());
                                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + player.getName() + " 因為伤害異常而被管理員永久停權。"));
                                World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] " + player.getName() + " (等級 " + player.getLevel() + ") " + "伤害異常。 " + "最高伤害 40 本次伤害 " + eachd + " 技能ID " + attack.skill));
                                player.ban(player.getName() + "伤害異常", true, true, false);
                                player.getClient().getSession().close();
...
//				    System.out.println("EXCEED!!! Client damage : " + eachd + " Server : " + MaxDamagePerHit);
                                    eachd = (int) (MaxDamagePerHit * 2); // Convert to server calculated damage
                                    player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_MAGIC_2, new StringBuilder().append("[伤害: ").append(eachd).append(", 預期: ").append(maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [职业: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 使用的技能: ").append(attack.skill).append("]").toString());
                                    if (eachd >= 10000) {
                                        if (ServerConfig.LOG_DAMAGE) {
                                            FileoutputUtil.logToFile("Logs/hack/伤害計算/魔法伤害計算修正_" + attack.skill + ".txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 职业: " + player.getJob() + " 怪物:" + monster.getId() + " 封包伤害 :" + eachd + " 預計伤害 :" + (int) maxDamagePerHit + "是否為BOSS: " + monster.getStats().isBoss(), false, false);
                                        }
                                        player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_MAGIC_2, new StringBuilder().append("[伤害: ").append(eachd).append(", 預期: ").append(maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [职业: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 使用的技能: ").append(attack.skill).append("]").toString());
                                        return;
                                    }
                ...
                if (range > SkillRange && !player.inBossMap()) { // 815^2 <-- the most ranged attack in the game is Flame Wheel at 815 range
                    player.getCheatTracker().registerOffense(CheatingOffense.ATTACK_FARAWAY_MONSTER, "攻击範圍異常,技能:" + attack.skill + "(" + SkillFactory.getName(attack.skill) + ")　正常範圍:" + (int) SkillRange + " 計算範圍:" + (int) range); // , Double.toString(Math.sqrt(distance))
                    if (range > SkillRange * 2) {
                        player.getCheatTracker().registerOffense(CheatingOffense.ATTACK_FARAWAY_MONSTER_BAN, "超大攻击範圍,技能:" + attack.skill + "(" + SkillFactory.getName(attack.skill) + ")　怪物:" + monster.getId() + " 正常範圍:" + (int) SkillRange + " 計算範圍:" + (int) range); // , Double.toString(Math.sqrt(distance))
                    }
                    return;
                ...
                if (attack.skill == SkillType.僧侶.群體治癒 && !monsterstats.getUndead()) {
                    ...
                    FileoutputUtil.logToFile("logs/Hack/Ban/技能異常.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家<" + player.getLevel() + ">: " + player.getName() + " 怪物 " + monster.getId() + " 地图: " + player.getMapId() + " 技能代码: " + attack.skill + " 使用群體治癒攻击非不死系怪物");
                    World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + player.getName() + " 因為技能異常而被管理員永久停權。"));
                    World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] " + player.getName() + " (等級 " + player.getLevel() + ") " + "技能異常。 使用群體治癒攻击非不死系怪物 技能ID " + attack.skill));
...
//      }
        ...
        switch (skill.getId()) {
            ...
            case SkillType.傳說.嫩寶丟擲術:
                ...
                break;
            case SkillType.冒險之技.法老的憤怒攻击:
            case SkillType.貴族.法老的憤怒攻击:
            case SkillType.傳說.法老的憤怒攻击:
                elemMaxDamagePerMob = 1;
                break;
        }
        ...
        if (theSkill != null) {
            ...
            switch (theSkill.getId()) {
                ...
                case SkillType.聖騎士.鬼神之擊://Heavens Hammer
                    ...
                    defined = true;
                    break;
                case SkillType.神偷.金币炸彈: // Meso Explosion
                    maximumDamageToMonster = (monster.getStats().isBoss() ? 500000 : monster.getMobMaxHp());
                    defined = true;
                    break;
...
//	}
        ...
        if (player.getDebugMessage()) {
            player.dropMessage("[伤害計算]屬性伤害：" + (int) Math.ceil(elementalMaxDamagePerMonster) + " BOSS伤害：" + (int) Math.ceil(((monster.getStats().isBoss()) ? player.getStat().bossdam_r : player.getStat().dam_r) - 100) + "%");
        }
    ...
    public static final AttackInfo Modify_AttackCrit(final AttackInfo attack, final MapleCharacter chr, final int type) {
        ...
        if (attack.skill != SkillType.神偷.金币炸彈
                && attack.skill != SkillType.狙擊手.寒冰箭
                && attack.skill != SkillType.暗殺者.金币攻击
...
//        lea.skip(4); //0
    ...
    public static final AttackInfo parseDmgM(final LittleEndianAccessor lea) {
        ...
        if (ret.skill == SkillType.神偷.金币炸彈) {
