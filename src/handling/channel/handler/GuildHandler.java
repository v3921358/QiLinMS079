...
public class GuildHandler {
    ...
    private static void SaveCharDb(final MapleCharacter c) {
        try {
            ...
            FileoutputUtil.outError("logs/公会存檔保存数据異常.txt", e);
        ...
        switch (operation) {
            ...
            case CREATE: {
                ...
                if (c.getPlayer().getGuildId() > 0 || c.getPlayer().getMapId() != 200000301) {
                    c.getPlayer().dropMessage(1, "無法建立公会\r\n已經有公会或不在英雄之殿");
                    return;
                ...
                if (!isGuildNameAcceptable(guildName)) {
                    c.getPlayer().dropMessage(1, "這個公会名稱是不被准許的.");
                    return;
                }
                ...
                World.Guild.setGuildMemberOnline(c.getPlayer().getMGC(), true, c.getChannel());
                c.getPlayer().dropMessage(1, "恭喜你成功創建一個公会.");
