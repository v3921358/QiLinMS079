            ...
            case CREATE: {
                ...
                if (createType == 3) { // trade
                    ...
                    if (createType == 1 || createType == 2) {
                        ...
                        if (World.isShutDown) {
                            chr.dropMessage(1, "伺服器即將关闭所以不能使用商店.");
                            c.sendPacket(MaplePacketCreator.enableActions());
                            return;
                        ...
                        for (int i = 1; i <= 5; i++) {
                            chr.dropMessage(1, "冒险岛精灵商人修復度滿高，但若真發生伺服器資料庫產生異常時有機率吃道具\r\n"
                                    + "並且這種若吃了一定会全部吃光\r\n"
                                    + "建議玩家擺少量商品，賣完再去補貨\r\n"
                                    + "你認定這道具若真的被吃了也沒關係，若發生被吃裝將不会給予任何補償\r\n"
                                    + "請同意了再擺攤商人，若不同意的玩家~請勿~擺攤，謝謝配合\r\n"
                                    + "很重要很重要的規則說5次。");
                        }
                        if (createType == 4) {
                ...
                if (chr.getTrade() != null) {
                    ...
                    if (chr.getTrade().getPartner() != null) {
                        ...
                        if (local.isLocked() && partners.isLocked()) {
                            ...
                            //chr.dropMessage(5, "雙方確認交易状态下，無法取消交易。");
                        } else {
                            MapleTrade.cancelTrade(chr.getTrade(), chr.getClient());
                        }
                    } else {
...
//                    }
                    if (ivItem.getQuantity() >= bundles_perbundle) {
                        ...
                        if (GameConstants.isThrowingStar(ivItem.getItemId()) || GameConstants.isBullet(ivItem.getItemId())) {
                            ...
                            FileoutputUtil.logToFile("logs/Data/精灵商人放入道具.txt", FileoutputUtil.NowTime() + "账号角色名字:" + c.getAccountName() + " " + c.getPlayer().getName() + " 道具： " + ivItem.getItemId() + " 數量:  " + bundles + "\r\n");
                        } else {
            ...
            case REMOVE_ITEM: {
                ...
                if (item != null) {
                    if (item.bundles > 0) {
                        ...
                        FileoutputUtil.logToFile("logs/Data/精灵商人道具拿出.txt", FileoutputUtil.NowTime() + "账号角色名字:" + c.getAccountName() + " " + c.getPlayer().getName() + " 道具： " + item_get.getItemId() + " 數量:  " + item_get.getQuantity() + "\r\n");
                    }
                }
                c.sendPacket(PlayerShopPacket.shopItemUpdate(shop));
                break;
            ...
            case CLOSE_MERCHANT: {
                try {
                    ...
                    if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr) && merchant.isAvailable()) {
                        ...
                        c.sendPacket(MaplePacketCreator.serverNotice(1, "請去找富蘭德里領取你的装备和金币"));
                        c.sendPacket(MaplePacketCreator.enableActions());
                    ...
                    FileoutputUtil.outError("logs/精灵商人关闭異常.txt", e);
                }
                break;
                /*  final IMaplePlayerShop merchant = chr.getPlayerShop();
