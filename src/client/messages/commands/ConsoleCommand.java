                ...
                if (ts == null && (t == null || !t.isAlive())) {
                    ...
                    ts = Timer.EventTimer.getInstance().register(new Runnable() {
                        ...
                        public void run() {
                            ...
                            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[楓之谷公告] 伺服器將在 " + minutesLeft + " 分鐘後关闭，請勿关闭精灵商人並存檔並下線。."));
                            System.out.println("伺服器將在 " + minutesLeft + "分鐘後关闭.");
                            minutesLeft--;
                        }
                    }, 60000);
                } else {
                    System.out.println("好吧真拿你沒辦法..伺服器关闭时间修改...請等待关闭完畢..請勿強制关闭服務器..否則後果自負!");
                }
            } else {
                System.out.println("使用規則: shutdowntime <关闭时间>");
                return 0;
            }
            return 1;
        }
 ...
 /*public static class ExpRate extends ConsoleCommandExecute {
    ...
    public static class Saveall extends ConsoleCommandExecute {
        ...
        public int execute(String[] splitted) {
            ...
            System.out.println("[保存] " + p + "個玩家数据保存到数据中.");
            return 1;
        }
    }
    ...
    public static class AutoReg extends ConsoleCommandExecute {
        ...
        public int execute(String[] splitted) {
            ...
            System.out.println("自動註冊状态: " + (LoginServer.getAutoReg() ? "开启" : "关闭"));
            return 1;
        }
    }
    ...
    public static class help extends ConsoleCommandExecute {
        ...
        public int execute(String[] splitted) {
            ...
            System.out.println("-------------------------");
            System.out.println("shutdown 关闭伺服器");
            System.out.println("shotdowntime <时间> 倒數关闭服務器");
            System.out.println("CrucialTime <时间> 關鍵時刻");
            System.out.println("reloadchannel 重新載入頻道");
            System.out.println("reloadmap 重新載入地图");
            System.out.println("Info 查看伺服器狀況");
            System.out.println("AutoReg 自動註冊开关");
            System.out.println("-------------------------");
