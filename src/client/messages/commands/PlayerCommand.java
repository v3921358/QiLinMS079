...
public class PlayerCommand {
    ...
    public static class npc extends 万能 {
        ...
        @Override
        public boolean execute(MapleClient c, String[] splitted) {
            NPCScriptManager.getInstance().start(c, 9330079, "拍卖功能");
            return true;
        }
        @Override
        public String getMessage() {
    ...
    public static class ea extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            c.sendPacket(MaplePacketCreator.sendHint(
                   ...
                   // + "经验值倍率 " + ((Math.round(c.getPlayer().getEXPMod()) * c.getPlayer().getExpm() * 100 * c.getChannelServer().getExpRate()) * Math.round(c.getPlayer().getStat().expBuff / 100.0) + (c.getPlayer().getStat().equippedFairy ? c.getPlayer().getFairyExp() : 0)) + "%, 爆率倍率 " + Math.round(c.getPlayer().getDropMod() * c.getPlayer().getDropm() * (c.getPlayer().getStat().dropBuff / 100.0) * 100 * c.getChannelServer().getDropRate()) + "%, 金币倍率 " + Math.round((c.getPlayer().getStat().mesoBuff / 100.0) * 100 * c.getChannelServer().getMesoRate()) + "% \r\n "
