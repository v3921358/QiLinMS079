...
//限制创建角色数量
    ...
    public final byte getLoginState() { // TODO hide?
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            loggedIn = false;
            FileoutputUtil.outError("logs/資料庫異常.txt", e);
            throw new DatabaseException("登入状态獲取失敗", e);
        }
    }
    ...
    public final void removalTask(boolean shutdown) {
        try {
           ...
           //     FileoutputUtil.outputFileError("logs/buff記錄时间異常.txt", e);
          //  }
        ...
        try {
            if (player != null) {
                ...
                try {
                    ...
                    FileoutputUtil.logToFile("logs/下線保存数据異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + getSession().remoteAddress().toString().split(":")[0] + " 账号 " + getAccountName() + " 账号ID " + getAccID() + " 角色名 " + player.getName() + " 角色ID " + player.getId());
                    FileoutputUtil.outError("logs/下線保存数据異常.txt", ex);
                }
                if (shutdown) {
                    player = null;
                    receiving = false;
    ...
    public final int deleteCharacter(final int cid) {
        ...
        FileoutputUtil.logToFile("Logs/Data/角色刪除.txt", FileoutputUtil.NowTime() + " 账号: " + accountName + "(" + this.accountId + ") 角色: " + cid + " (" + name + ") IP: " + getSessionIPAddress() + " \r\n");
    ...
    public final void unLockDisconnect() {
        getSession().writeAndFlush(MaplePacketCreator.serverNotice(1, "當前账号在別處登入\r\n若不是你本人操作請及時更改密碼。"));
    ...
    public boolean hasCheck(int accid) {
        ...
        try {
            ...
            System.err.println("Error checking ip Check" + ex);
        }
        return ret;
    }


   /* public String 获取登录器值() {
        return this.登录器值;
    }

    public String 获取数据库对比值() { //数据库对比的值
        return this.数据库对比值;
    }*/
