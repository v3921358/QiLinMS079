                ...
                if (((!MapleLieDetector.this.isPassed()) /*&& (!isItem)*/) && (searchchr != null)) {
                    if (MapleLieDetector.this.attempt >= 3) {
                        ...
                        if ((search_chr != null) && (search_chr.getId() != searchchr.getId())) {
                            ...
                            //FileoutputUtil.logToFile("logs/Data/測謊失敗.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + searchchr.getClient().getSession().remoteAddress().toString().split(":")[0] + " 账号: " + searchchr.getClient().getAccountName() + " 玩家: " + searchchr.getClient().getPlayer().getName() + " 沒有通過測謊儀。");
                        }
                        ...
                        World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 玩家: " + searchchr.getName() + " (等級 " + searchchr.getLevel() + ") 未通過測謊儀檢測，疑似使用腳本外掛！"));
                    } else {
