...
public class MapleSquad {
        ...
        while (type.queue.get(ch).size() > 0 && ChannelServer.getInstance(ch).getMapleSquad(type) == null) {
            ...
            if (theirCh > 0) {
                ...
                if (lead != null && lead.getMapId() == beginMapId && lead.getClient().getChannel() == ch) {
                    ...
                    getBeginMap().broadcastMessage(MaplePacketCreator.serverNotice(6, nextPlayerId + "遠征队已經結束了，由於有成員沒有在地图內"));
                    type.queuedPlayers.get(ch).add(new Pair<>(nextPlayerId, "不在地图內"));
                }
            } else {
                getBeginMap().broadcastMessage(MaplePacketCreator.serverNotice(6, nextPlayerId + "'遠征队已經結束了，由於有成員沒有在線上"));
    ...
    public String getNextPlayer() {
        ...
        sb.append("#b").append(type.queue.get(ch).size()).append(" #k ").append("與遠征队名單 : \n\r ");
        int i = 0;
        ...
        if (join) {
            if (!containsMember(member) && !getAllNextPlayer().contains(member.getName())) {
                if (members.size() <= 30) {
                    ...
                    getLeader().dropMessage(6, member.getName() + " (" + job + ") 加入了遠征队!");
                    return 1;
                }
                return 2;
            }
            ...
            if (containsMember(member)) {
                ...
                getLeader().dropMessage(6, member.getName() + " (" + job + ") 離開了遠征队.");
                return 1;
            }
            return -1;
        }
    ...
    public void acceptMember(int pos) {
        ...
        if (toadd != null && getChar(toadd) != null) {
            ...
            getChar(toadd).dropMessage(5, getLeaderName() + " 允許你重新回來遠征队");
        }
    }
    ...
    public void banMember(int pos) {
        ...
        if (toban != null && getChar(toban) != null) {
            ...
            getChar(toban).dropMessage(5, getLeaderName() + " 從遠征队中刪除您.");
        }
    }
    ...
    public String getSquadMemberString(byte type) {
        switch (type) {
            case 0: {
                StringBuilder sb = new StringBuilder("目前遠征队總人數 : ");
