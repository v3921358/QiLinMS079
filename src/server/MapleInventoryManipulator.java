...
//            c.setMonitored(true);
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (reqlv > c.getPlayer().getLevel() + c.getPlayer().getStat().levelBonus && !c.getPlayer().isGM()) {
            FileoutputUtil.logToFile("logs/Hack/Ban/修改封包.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家：" + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") <等級: " + c.getPlayer().getLevel() + " > 修改装备(" + source.getItemId() + ")封包，穿上装备時封鎖。 該装备需求等級: " + reqlv);
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + c.getPlayer().getName() + " 因為修改封包而被管理員永久停權。"));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语]  " + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") <等級: " + c.getPlayer().getLevel() + " > 修改装备(" + source.getItemId() + ")封包，穿上装备時封鎖。 該装备需求等級: " + reqlv));
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        if (c.getPlayer().getDebugMessage()) {
            c.getPlayer().dropMessage("脫装备: src : " + src + " dst : " + dst + " 代码：" + source.getItemId());
        }
        if (source.getItemId() == 1122017 || source.getItemId() == 1122086 || source.getItemId() == 1122207 || source.getItemId() == 1122215) {
        ...
        int reqlv = MapleItemInformationProvider.getInstance().getReqLevel(source.getItemId());
        if (reqlv > c.getPlayer().getLevel() + c.getPlayer().getStat().levelBonus && !c.getPlayer().isGM()) {
            FileoutputUtil.logToFile("logs/Hack/Ban/修改封包.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家：" + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") <等級: " + c.getPlayer().getLevel() + " > 修改装备(" + source.getItemId() + ")封包，脫除装备時封鎖。 該装备需求等級: " + reqlv);
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + c.getPlayer().getName() + " 因為修改封包而被管理員永久停權。"));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语]  " + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") <等級: " + c.getPlayer().getLevel() + " > 修改装备(" + source.getItemId() + ")封包，脫除装备時封鎖。 該装备需求等級: " + reqlv));
            c.getPlayer().ban("修改封包", true, true, false);
            c.getSession().close();
            return;
        }
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced, final boolean cs) {
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
     //   if (World.isShutDown) {
        int 丢出物品开关 = gui.Qhms.ConfigValuesMap.get("丢出物品开关");
        if (丢出物品开关 > 0) {
        ...
        if (ChrdangerousIp) {
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 危險IP丟棄物品 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + c.getPlayer().getName() + " 角色ID " + c.getPlayer().getId() + " 類型 " + type + " src " + src + " 數量 " + quantity));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 危險IP丟棄物品 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + c.getPlayer().getName() + " 角色ID " + c.getPlayer().getId() + " 類型 " + type + " src " + src + " 數量 " + quantity));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 危險IP丟棄物品 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + c.getPlayer().getName() + " 角色ID " + c.getPlayer().getId() + " 類型 " + type + " src " + src + " 數量 " + quantity));
            FileoutputUtil.logToFile("logs/Data/危險IP丟棄物品.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + c.getPlayer().getName() + " 角色ID " + c.getPlayer().getId() + " 類型 " + type + " src " + src + " 數量 " + quantity);
        }*/
        ...
        try {
            ...
            if (ChrdangerousName) {
                if (WorldConstants.DropItem) {
                    World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 危險账号丟棄物品 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + c.getPlayer().getName() + " 角色ID " + c.getPlayer().getId() + " 類型 " + type + " src " + src + quantity + " 物品 " + ii.getName(source.getItemId()) + " (" + source.getItemId() + ") x" + quantity));
                }
                FileoutputUtil.logToFile("logs/Data/危險账号丟棄物品.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + c.getPlayer().getName() + " 角色ID " + c.getPlayer().getId() + " 類型 " + type + " src " + src + quantity + " 物品 " + ii.getName(source.getItemId()) + " (" + source.getItemId() + ") x" + quantity);
            }
            ...
            if ((source.getItemId() / 10000 == 204) || (source.getItemId() == 2340000) || (source.getItemId() == 2022179) || (GameConstants.getInventoryType(source.getItemId()) == MapleInventoryType.SETUP)) {
                if (WorldConstants.DropItem) {
                    World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系統] 危險貴重物品 账号 " + c.getAccountName() + " 账号ID " + c.getAccID() + " 角色名 " + c.getPlayer().getName() + " 角色ID " + c.getPlayer().getId() + " 類型 " + type + " src " + src + quantity + " 物品 " + ii.getName(source.getItemId()) + " (" + source.getItemId() + ") x" + quantity));
                }
    ...
    public static void removeAllByEquipOnlyId(MapleClient c, long inventoryitemid) {
        ...
        if (copyEquipItems != null) {
            ...
            String msgtext = "玩家" + c.getPlayer().getName() + " ID: " + c.getPlayer().getId() + " (等級" + c.getPlayer().getLevel() + ") 地图: " + c.getPlayer().getMapId() + " 在玩家背包中發現复制装备[" + ii.getName(copyEquipItems.getItemId()) + "]已經將其刪除。";
        ...
        if (copyEquipedItems != null) {
            ...
            String msgtext = "玩家" + c.getPlayer().getName() + " ID: " + c.getPlayer().getId() + " (等級" + c.getPlayer().getLevel() + ") 地图: " + c.getPlayer().getMapId() + " 在玩家穿戴中發現复制装备[" + ii.getName(copyEquipedItems.getItemId()) + "]已經將其刪除。";
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] " + msgtext));
