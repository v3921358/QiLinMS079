...
public class HiredMerchant extends AbstractPlayerStore {
    ...
    public void buy(MapleClient c, int item, short quantity) {
        ...
        if (MapleInventoryManipulator.addFromDrop(c, newItem, false)) {
            ...
            if (Owner != null) {
                Owner.dropMessage(5, "道具 " + MapleItemInformationProvider.getInstance().getName(newItem.getItemId()) + " (" + perbundle + ") × " + quantity + " 已被其他玩家购买，還剩下：" + pItem.bundles + " 個");
            }
            ...
            if (ServerConfig.LOG_MRECHANT) {
                FileoutputUtil.logToFile("logs/Data/精灵商人.txt", "\r\n 时间　[" + FileoutputUtil.NowTime() + "] IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 玩家 " + c.getAccountName() + " " + c.getPlayer().getName() + " 從  " + getOwnerName() + " 的精灵商人购买了" + MapleItemInformationProvider.getInstance().getName(newItem.getItemId()) + " (" + newItem.getItemId() + ") x" + quantity + " 單個價錢為 : " + pItem.price);
            }
            final StringBuilder sb = new StringBuilder("[GM 密语] 玩家 " + c.getPlayer().getName() + " 從  " + getOwnerName() + " 的精灵商人购买了 " + MapleItemInformationProvider.getInstance().getName(newItem.getItemId()) + "(" + newItem.getItemId() + ") x" + quantity + " 單個價錢為 : " + pItem.price);
    ...
    public void closeShop(boolean saveItems, boolean remove) {
        try {
            ...
            FileoutputUtil.outError("logs/精灵商人关闭異常.txt", se);
        }
    }
