...
public class MapleTrade {
    ...
    public final void CompleteTrade() {
        ...
        if (exchangeItems != null) { // just to be on the safe side...
            ...
            if ((exchangeMeso - GameConstants.getTaxAmount(exchangeMeso)) >= 50000000) {
                FileoutputUtil.logToFile("logs/Data/大額金幣交易.txt", FileoutputUtil.NowTime() + "角色名字:" + chr.get().getName() + " 和 " + partner.chr.get().getName() + " 交易获得 金幣" + (exchangeMeso - GameConstants.getTaxAmount(exchangeMeso)) + "\r\n");
                World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] " + "大額金幣交易 疑似賣游戏币 角色名字:" + chr.get().getName() + " 和 " + partner.chr.get().getName() + " 交易获得 金幣" + (exchangeMeso - GameConstants.getTaxAmount(exchangeMeso))));
            }

            FileoutputUtil.logToFile("logs/Data/交易記錄.txt", FileoutputUtil.NowTime() + " 账号角色名字:" + chr.get().getClient().getAccountName() + " " + chr.get().getName() + " 和 " + partner.chr.get().getClient().getAccountName() + " " + partner.chr.get().getName() + " 交易获得 金幣" + (exchangeMeso - GameConstants.getTaxAmount(exchangeMeso)) + " 和 " + exchangeItems.size() + "件物品[" + output + "]\r\n");
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] " + "角色名字:" + chr.get().getName() + " 和 " + partner.chr.get().getName() + " 交易获得 金幣" + (exchangeMeso - GameConstants.getTaxAmount(exchangeMeso)) + " 和 " + exchangeItems.size() + "件物品[" + output + "]"));
            exchangeItems.clear();
        }
        if (exchangeMeso > 0) {
        ...
        try {
            ...
            FileoutputUtil.logToFile("logs/交易存檔保存数据異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + chr.get().getClient().getSession().remoteAddress().toString().split(":")[0] + " 账号 " + chr.get().getClient().getAccountName() + " 账号ID " + chr.get().getClient().getAccID() + " 角色名 " + chr.get().getName() + " 角色ID " + chr.get().getId());
            FileoutputUtil.outError("logs/交易存檔保存数据異常.txt", ex);
            System.err.println("封鎖出現錯誤 " + ex);
            FileoutputUtil.outError("logs/交易異常.txt", ex);
        }
    }
    ...
    public final void chat(final String message) {
        ...
        if (!CommandProcessor.processCommand(chr.get().getClient(), message, CommandType.TRADE)) {
            ...
            final StringBuilder sb = new StringBuilder("[GM 密语] 『" + chr.get().getName() + "』對『" + partner.getChr().getName() + "』的交易聊天：  " + message);
    ...
    public static final void inviteTrade(final MapleCharacter c1, final MapleCharacter c2) {
        //if (World.isShutDown) {
          ...
          //  return;
       // }
       int 玩家交易开关 = gui.Qhms.ConfigValuesMap.get("玩家交易开关");
        if (玩家交易开关 > 0) {
