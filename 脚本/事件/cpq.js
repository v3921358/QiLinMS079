...
function registerCarnivalParty(eim, carnivalParty) {
    if (eim.getProperty("red").equals("-1")) {
		...
		eim.broadcastPlayerMsg(5, "接下來的三分钟您的队伍可以寻找其他人挑战。");
    } else {
...
function check(eim) {
	var ck = eim.check();
	if(ck) {
		eim.broadcastPlayerMsg(5, "检测中..目前无异常....!");
		eim.schedule("start", 10000);
		eim.broadcastPlayerMsg(5, "10秒后將开战！！");
	} else {
...
function playerDisconnected(eim, player) {
    ...
    eim.unregisterPlayer(player);
    player.getCarnivalParty().removeMember(player);
    eim.broadcastPlayerMsg(5, player.getName() + " 离开了怪物擂台1");
    disposeAll(eim);
}
