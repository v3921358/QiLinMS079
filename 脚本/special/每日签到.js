...
function action(mode, type, selection) {
    ...
    switch (select) {
        ...
        case 1:
        {
		if (cm.getBossLog("每日签到30") >= 1 || cm.getGamePoints() <= 30  || cm.getPlayerStat("LVL") < 30 ) {
            cm.sendOk("签到失败原因：\r\n1.您今天已经签到过了\r\n2.您的在线时间不足30分钟\r\n3.您的等级不足30级\r\n谢谢光临，请明天再过來。");
			cm.dispose();	
        }else{
            cm.setBossLog("每日签到30");
		...
		case 2:
        {
		if (cm.getBossLog("每日签到70") >= 1 || cm.getGamePoints() <= 60  || cm.getPlayerStat("LVL") < 70 ) {//|| cm.getGamePoints() < 60
            cm.sendOk("签到失败原因：\r\n1.您今天已经签到过了\r\n2.您的在线时间不足60分钟\r\n3.您的等级不足70级\r\n谢谢光临，请明天再过來。");
			cm.dispose();	
        }else{
            cm.setBossLog("每日签到70");
			//cm.serverNotice("『公告』：玩家【"+ cm.getChar().getName() +"】在自由市场 领取了每日奖励。");
			//cm.gainItem(2049100,5);//混沌卷轴
			//cm.gainItem(5072000,3);//高质地喇叭
		...
		case 3:
        {
		if (cm.getBossLog("每日签到120") >= 1 || cm.getGamePoints() <= 120  || cm.getPlayerStat("LVL") < 120 ) {
            cm.sendOk("签到失败原因：\r\n1.您今天已经签到过了\r\n2.您的在线时间不足120分钟\r\n3.您的等级不足120级\r\n谢谢光临，请明天再过來。");
			cm.dispose();	
        }else{
            cm.setBossLog("每日签到120");
			//cm.serverNotice("『公告』：玩家【"+ cm.getChar().getName() +"】在自由市场 领取了每日奖励。");
			//cm.gainItem(2049100,5);//混沌卷轴
			//cm.gainItem(5072000,3);//高质地喇叭
		...
		case 4:
        {
		if (cm.getBossLog("每日签到150") >= 1 || cm.getGamePoints() <= 120  || cm.getPlayerStat("LVL") < 150 ) {
            cm.sendOk("签到失败原因：\r\n1.您今天已经签到过了\r\n2.您的在线时间不足120分钟\r\n3.您的等级不足150级\r\n谢谢光临，请明天再过來。");
			cm.dispose();	
        }else{
            cm.setBossLog("每日签到150");
			//cm.serverNotice("『公告』：玩家【"+ cm.getChar().getName() +"】在自由市场 领取了每日奖励。");
			//cm.gainItem(2049100,5);//混沌卷轴
			//cm.gainItem(5072000,3);//高质地喇叭
        ...
        case 5:
        {
		if (cm.getBossLog("每日签到200") >= 1 || cm.getGamePoints() <= 120  || cm.getPlayerStat("LVL") < 200 ) {
            cm.sendOk("签到失败原因：\r\n1.您今天已经签到过了\r\n2.您的在线时间不足120分钟\r\n3.您的等级不足200级\r\n谢谢光临，请明天再过來。");
			cm.dispose();	
        }else{
            cm.getBossLog("每日签到200");
			//cm.serverNotice("『公告』：玩家【"+ cm.getChar().getName() +"】在自由市场 领取了每日奖励。");
			//cm.gainItem(2049100,5);//混沌卷轴
			//cm.gainItem(5072000,3);//高质地喇叭
