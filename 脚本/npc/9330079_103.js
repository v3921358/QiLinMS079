...
function action(mode, type, selection) {
		...
		if (status == 0) {
			var text = "我是饱读诗书的孔明，答题时间为每时#r49-59分#n#k。\r\n每日答题上限100题请认真回答每一题~#b\r\n";
			text += "服务器当前时间：#r" + hour +":" + minute + ":" + second + " （每题限时20秒，超时作废）\r\n#n#k\r\n\r\n";
			text += "今日答题总数: #r"+ cm.getPlayer().getBossLog("答题总数") +" #b道#n#k\r\n";
			text += "今日正确题目: #r"+ cm.getPlayer().getBossLog("答题正确") +" #b道#n#k\r\n";
			
			text += "#L2##b 开始答题（"+timeout+"秒限时，速度要快哦）#l\r\n\r\n";
			text += "#L1##b 点此查看答题详细规则（新手必看）#l\r\n\r\n";
			text += "#L3##r 答对题目数兑换/BOSS币/余额/点卷#l\r\n\r\n";
			text += "#b    <注意>:每天晚上零点#r清空答题数据#b，请及时兑换\r\n";
			cm.sendSimple(text);
		}else if (status == 1) {
			if(selection==1){
					cm.sendOk("每次答题都有#b"+timeout+"秒钟时间限制#k，超过时间则失败，指定时间内回答错误也失败！点击开始答题后开始记时，在给出的题目中，#r一般有4个选项#k，请输入你认为正确的选项，点击确认即可完成答题.\r\n#r#e注意：每小时的49分，#b如（1点49分至1点59分）#r,都可以答题，答题时间为10分钟。每次回答正确后，将会#d\r\n随机给你奖励，如冒险币、点卷、抵用券、喇叭或是运动会币！");
					cm.dispose();
					return;
			}
			if(selection==2){
			...
			if(selection==3){// 38道 1国庆币    58道 1暗影币      68道 1祝福卷     78道 1混沌   88道勋章
				typed = 2;
				var selText = "截至目前为止，您答对的题目总数为:#r"+cm.getBossLog("答题正确") +"道#b\r\n";
				selText += "#L1# 换取 国庆币#v4000463# 1个     (需答对 38道)#l\r\n";
				selText += "#L2# 换取 暗影币#v4000487# 1个    (需答对 58道)#l\r\n";
				selText += "#L3# 换取 祝福卷轴#v2340000# 1个  (需答对 68道)#l\r\n";
				selText += "#L4# 换取 混沌卷轴#v2049100# 1个  (需答对 78道)#l\r\n";
				selText += "#L5# 换取 #v1142072#(全属性+15 500HP/MP) 1个 (需答对 88道)#l\r\n\r\n\r\n";
			...
			if(typed == 1){
				...
				if(selection == tknow[5] || fee == tknow[5]){
					...
					if(randx < 3){//冒险币
						...
						cm.喇叭(1, "[每日答题]：恭喜：" + cm.getPlayer().getName() + "正确回答<诸葛孔明>的问题,得到 " +randy+"冒险币~");
					}
					...
					if(randx >= 9 && randx < 12){//喇叭
						...
						cm.sendOk("恭喜，您答对了！#b正确答案:"+tknow[5]+"#k\r\n您获得了 " +randjf+" 高质地喇叭.");
						cm.喇叭(1, "[每日答题]：恭喜：" + cm.getPlayer().getName() + "正确回答<诸葛孔明>的问题,得到 " +randjf+"高置地喇叭~");
					}
					if(randx >= 12 && randx<= 15){//征服币
						cm.gainItem(4310030, randred);
						cm.sendOk("恭喜，您答对了！#b正确答案:"+tknow[5]+"#k\r\n您获得了 " +randred+" 运动会币.");
					        cm.喇叭(1, "[每日答题]：恭喜：" + cm.getPlayer().getName() + "正确回答<诸葛孔明>的问题,得到 " +randred+"运动会币~");
					}					
				}else{
					cm.sendOk("很遗憾，回答错误！\r\n");
			...
			if(typed==2){
				if(selection==1){//// 38道 1国庆币4000463   
					if (cm.getPlayer().getBossLog("答题38兑换") < 1){
						...
						cm.gainItem(4000463, 1);//天璇
						//cm.gainItem(4310038, 5);//天璇
						//cm.gainItem(2049100, 2);//祝福卷
						cm.getPlayer().setBossLog("答题38兑换");
						cm.sendOk("恭喜，您使用正确答题数目兑换成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目38题兑换了 国庆币~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
					}
				} else if (selection == 2){//58道 1暗影币 4000487 
						...
						cm.gainItem(4000487, 1);//1暗影币
						//cm.gainItem(4310038, 5);//天璇
						//cm.gainItem(2049100, 2);//祝福卷
										//cm.setmoneyb(50);//奖励充值币
						cm.getPlayer().setBossLog("答题58兑换");
						cm.sendOk("恭喜，您使用正确答题数目兑换成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目58题兑换了 暗影币~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
					}
				} else if (selection == 3){// 68道 1祝福卷2340000   78道 1混沌2049100   88道勋章
						...
						cm.sendOk("恭喜，您使用正确答题数目兑换成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目68题兑换了 祝福卷轴~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
					}
					} else if (selection == 4){//   78道 1混沌2049100   88道勋章
						...
						cm.sendOk("恭喜，您使用正确答题数目兑换成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目78题兑换了 混沌卷轴~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
						...
						cm.gainItem(1142072,15,15,15,15,500,500,10,10,15,15,15,15,15,15);//道勋章1142609
						
						cm.getPlayer().setBossLog("答题88兑换");
						cm.sendOk("恭喜，您使用正确答题数目兑换成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目88题兑换了 答题勋章(全属性+20)~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
