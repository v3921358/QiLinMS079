...
var chance5 = Math.floor(Math.random() * 100 + 50);//抵用卷随机变量
...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == 180000001) {
        ...
        switch (selection) {
        case 1:
		    //cm.sendOk("         #r每周末#d（星期五、星期六、星期天）#k晚上8点30分至8点35分来找我参与挤牛奶活动吧，丰厚点卷大量魔方卷轴等你来抢哦#k");

            cm.sendOk("         #r活动将在每天的20点30分-35分开始#n\r\n\r\n#b开启活动后请疯狂点击我。这样您将随机获得一些点卷，抵用卷，金币的奖励。#k");
            cm.dispose();
            break;
        case 2:
            if ( ( hour == 20 && minute >= 30 && minute <= 35 ) ) {//开启时间每小时23-24分钟之间
		   //if(hour == 12 && (minute >= 10 && minute <= 35) && (weekday == 6 || weekday == 5 || weekday == 0)){// || cm.getPlayer().getName() == "管理员卡菲"){
                cm.gainNX(chance4);//给与玩家随机变量点卷
                cm.gainDY(chance5);//给与玩家随机变量低佣卷
                cm.gainMeso(chance6);//给与玩家随机变量游戏币
                //cm.gainItem(4001839, chance7);//给与玩家随机变量强化星星
                //cm.gainItem(4310036, chance8);//给与玩家随机变量征服者币
                cm.sendOk("你挤出 #r" + chance4 + " 点卷.\r\n" + chance5 + "#k 抵用卷\r\n#r" + chance6 + "#k 游戏币\r\n");
                //cm.worldSpouseMessage(0x19, "『疯狂挤奶』 : 恭喜 " + cm.getChar().getName() + " 在市场 11 洞门口奶牛挤出 " + chance4 + " 点卷. " + chance5 + " 点抵用卷 . " + chance6 + " 游戏币. ");//【活动翻倍】
	         //cm.worldSpouseMessage(0x20, "『【活动翻倍】疯狂挤奶』 : 恭喜 " + cm.getChar().getName() + " 在市场 12 洞门口奶牛挤出 " + chance4 + " 点卷.");//【活动翻倍】
                cm.dispose();
            } else {
